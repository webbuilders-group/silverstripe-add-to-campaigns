...
class AddToCampaignController extends LeftAndMain
{
    ...
    public function addtocampaign($data, $form)
    {
        ...
        if (empty($modelClass) || !class_exists($modelClass) || !in_array($modelClass, $this->config()->campaignable_classes) || !$modelClass::has_extension(Versioned::class)) {
            $this->httpError(404, _t(__CLASS__ . '.ErrorNotFound', 'That {Type} couldn\'t be found', '', ['Type' => $modelClass]));
        ...
        if ($this->getSchemaRequested()) {
            ...
            $extraData = [
                'message' => ($results instanceof HTTPResponse ? $results->getBody() : $results),
            ];
    ...
    public function getAddToCampaignForm($id, $modelClass = null)
    {
        ...
        if (!$record) {
            $this->httpError(404, _t(__CLASS__ . '.ErrorNotFound', 'That {Type} couldn\'t be found', '', ['Type' => $modelClass::singleton()->i18n_singular_name()]));

            return null;
        }

        if (!$record->canView()) {
            $this->httpError(403, _t(__CLASS__ . '.ErrorItemPermissionDenied', 'It seems you don\'t have the necessary permissions to add {ObjectTitle} to a campaign', '', ['ObjectTitle' => $modelClass::singleton()->i18n_singular_name()]));
